...
class SetupTechnoHD(ConfigListScreen, Screen):
    ...
    <screen name="SetupTechnoHD" position="0,0" size="1280,720" title=" " flags="wfNoBorder">
        <ePixmap position="0,0" zPosition="-1" size="1280,720" pixmap="TechnoHD/style/greymenu_1.png" />
        ...
        <ePixmap position="775,450" zPosition="1" size="500,150" pixmap="TechnoHD/logo.png" alphatest="blend" />
        <widget name="config" position="50,95" size="710,540" scrollbarMode="showOnDemand" selectionPixmap="TechnoHD/style/greysel.png" transparent="1" />
        ...
        <ePixmap pixmap="TechnoHD/buttons/key_info.png" position="800,615" size="35,25" zPosition="2" alphatest="blend" />
        <widget source="session.VideoPicture" render="Pig" position="806,96" size="428,248" zPosition="3" backgroundColor="transparent" />
        <ePixmap position="51,651" size="281,16" pixmap="TechnoHD/buttons.png" alphatest="blend" zPosition="1" />
        <ePixmap position="350,651" size="281,16" pixmap="TechnoHD/buttons.png" alphatest="blend" zPosition="1" />
        <ePixmap position="648,651" size="281,16" pixmap="TechnoHD/buttons.png" alphatest="blend" zPosition="1" />
        <ePixmap position="946,651" size="281,16" pixmap="TechnoHD/buttons.png" alphatest="blend" zPosition="1" />
        <ePixmap pixmap="TechnoHD/buttons/red2.png" position="54,653" size="275,10" alphatest="on" zPosition="2" />
        <ePixmap pixmap="TechnoHD/buttons/green2.png" position="353,653" size="275,10" alphatest="on" zPosition="2" />
        <ePixmap pixmap="TechnoHD/buttons/yellow2.png" position="650,653" size="275,10" alphatest="on" zPosition="2" />
        <ePixmap pixmap="TechnoHD/buttons/blue2.png" position="949,653" size="275,10" alphatest="on" zPosition="2" />
    </screen>"""
    ...
    def save(self):
#        skinpath = "/usr/share/enigma2/TechnoHD/"
    # save config
    ...
    def createSkin(self):
        SKIN = "/usr/share/enigma2/TechnoHD/skin.xml"
        DEFSKIN = "/usr/share/enigma2/TechnoHD/defskin.xml"
        try:
    # default skin
            skin_default = []
#            system("cp %sdefskin.xml %sskin.xml" % (skinpath, skinpath))
    # color`s text
            skin_default.append(["#10ffcc33", config.skin.techno.titlecolor.value])
            skin_default.append(["#10f4f4f4", config.skin.techno.textcolor.value])
            skin_default.append(["#108f8f8f", config.skin.techno.avtextcolor.value])
            skin_default.append(["#100099ff", config.skin.techno.textcurcolor.value])
#            system("sed -i 's/#10ffcc33/%s/w' %sskin.xml" % (config.skin.techno.titlecolor.value, skinpath))
#            system("sed -i 's/#10f4f4f4/%s/w' %sskin.xml" % (config.skin.techno.textcolor.value, skinpath))
#            system("sed -i 's/#108f8f8f/%s/w' %sskin.xml" % (config.skin.techno.avtextcolor.value, skinpath))
#            system("sed -i 's/#100099ff/%s/w' %sskin.xml" % (config.skin.techno.textcurcolor.value, skinpath))
    # fonts
#            system("sed -i 's/LiberationSans-Regular/%s/w' %sskin.xml" % (config.skin.techno.fonts.value, skinpath))
            skin_default.append(["LiberationSans-Regular", config.skin.techno.fonts.value])
    # number channel
#            system("sed -i 's/%s/TemplatesNumberCh/w' %sskin.xml" % (config.skin.techno.numberchannel.value, skinpath))
            skin_default.append([config.skin.techno.numberchannel.value, "TemplatesNumberCh"])
    # ecm-epg panel
#            system("sed -i 's/%s/TemplatesInfoBarECM-EPG/w' %sskin.xml" % (config.skin.techno.ecmepgpanel.value, skinpath))
            skin_default.append([config.skin.techno.ecmepgpanel.value, "TemplatesInfoBarECM-EPG"])
    # epg channel selection
#            system("sed -i 's/%s/TemplatesChannelSelectionInfoEPG/w' %sskin.xml" % (config.skin.techno.epgchannelselection.value, skinpath))
            skin_default.append([config.skin.techno.epgchannelselection.value, "TemplatesChannelSelectionInfoEPG"])
    # info channel selection
#            system("sed -i 's/%s/TemplatesChannelSelectionInfoChannel/w' %sskin.xml" % (config.skin.techno.infochannelselection.value, skinpath))
            skin_default.append([config.skin.techno.infochannelselection.value, "TemplatesChannelSelectionInfoChannel"])
    # cover info panel
#            system("sed -i 's/%s/TemplatesInfoBarInfoMovie-Cover/w' %sskin.xml" % (config.skin.techno.coverinfopanel.value, skinpath))
            skin_default.append([config.skin.techno.coverinfopanel.value, "TemplatesInfoBarInfoMovie-Cover"])
    # info movie selection
#            system("sed -i 's/%s/TemplatesMovieSelectionInfoMovie/w' %sskin.xml" % (config.skin.techno.infomovieselection.value, skinpath))
            skin_default.append([config.skin.techno.infomovieselection.value, "TemplatesMovieSelectionInfoMovie"])
    # clock panel menu
#            system("sed -i 's/%s/Clock/w' %sskin.xml" % (config.skin.techno.clockpanel.value, skinpath))
            skin_default.append([config.skin.techno.clockpanel.value, "Clock"])
    # dish
#            system("sed -i 's/%s/Dish/w' %sskin.xml" % (config.skin.techno.dish.value, skinpath))
            skin_default.append([config.skin.techno.dish.value, "Dish"])
    # scrollbar
#            system("sed -i 's/showNever/%s/w' %sskin.xml" % (config.skin.techno.scrollbarmode.value, skinpath))
            skin_default.append(["showNever", config.skin.techno.scrollbarmode.value])
    # style progress
#            system("sed -i 's/yellowprogress/%sprogress/w' %sskin.xml" % (config.skin.techno.progresscolor.value, skinpath))
#            system("sed -i 's/yellowprogress_big/%sprogress_big/w' %sskin.xml" % (config.skin.techno.progresscolor.value, skinpath))
            skin_default.append(["yellowprogress", config.skin.techno.progresscolor.value + "progress"])
            skin_default.append(["yellowprogress_big", config.skin.techno.progresscolor.value + "progress_big"])
    # style skin`s
#            system("sed -i 's/greyinfobar/%sinfobar/w' %sskin.xml" % (config.skin.techno.style.value, skinpath))
#            system("sed -i 's/greymenu/%smenu/w' %sskin.xml" % (config.skin.techno.style.value, skinpath))
#            system("sed -i 's/greysel/%ssel/w' %sskin.xml" % (config.skin.techno.style.value, skinpath))
#            system("sed -i 's/greyvolume/%svolume/w' %sskin.xml" % (config.skin.techno.style.value, skinpath))
#            system("sed -i 's/greydownload/%sdownload/w' %sskin.xml" % (config.skin.techno.style.value, skinpath))

            skin_default.append(["greyinfobar", config.skin.techno.style.value + "infobar"])
            skin_default.append(["greymenu", config.skin.techno.style.value + "menu"])
            skin_default.append(["greysel", config.skin.techno.style.value + "sel"])
            skin_default.append(["greyvolume", config.skin.techno.style.value + "volume" ])
            skin_default.append(["greydownload", config.skin.techno.style.value + "download" ])
    # end
    ...
    def install(self):
        ...
        try:
    # install converter
            system("cp %sSetupTechnoHD/components/AlwaysTrue.py %sConverter/AlwaysTrue.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/CaidInfo2.py %sConverter/CaidInfo2.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/CamdInfo3.py %sConverter/CamdInfo3.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/ConverterRotator.py %sConverter/ConverterRotator.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/EventName2.py %sConverter/EventName2.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/FrontendInfo2.py %sConverter/FrontendInfo2.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/ProgressDiskSpaceInfo.py %sConverter/ProgressDiskSpaceInfo.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/RouteInfo.py %sConverter/RouteInfo.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/ServiceInfoEX.py %sConverter/ServiceInfoEX.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/ServiceName2.py %sConverter/ServiceName2.py" % (pluginpath, componentspath))
    # install renderer
            system("cp %sSetupTechnoHD/components/PiconUni.py %sRenderer/PiconUni.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/RendVolumeText.py %sRenderer/RendVolumeText.py" % (pluginpath, componentspath))
            system("cp %sSetupTechnoHD/components/Watches.py %sRenderer/Watches.py" % (pluginpath, componentspath))
    # end
        except:
            self.session.open(MessageBox, _("Error by processing !!!"), MessageBox.TYPE_ERROR)
        self.session.openWithCallback(self.restart, MessageBox,_("Do you want to restart the GUI now ?"), MessageBox.TYPE_YESNO)
    ...
    def restart(self, answer):
        if answer is True:
            config.skin.primary_skin.value = 'TechnoHD/skin.xml'
